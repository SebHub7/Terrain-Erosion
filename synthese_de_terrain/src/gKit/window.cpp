...
int run( Window window, int (*draw)() )
{
    ...
    bool temp;
    int temp1, temp2;
    while(events(window, temp, temp1, temp2))
    {
        // dessiner
        if(draw() < 1)
...
static int e_pressed = false;
int GetEPressed() { return e_pressed; }

int events( Window window, bool& added_point, int& mousex, int& mousey)
{
